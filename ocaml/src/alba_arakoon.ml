...
let config_from_url url =
  let fetcher =
    try
      let protocol, rest = Scanf.sscanf url "%s@://%s" (fun p r -> (p,r)) in
      match protocol with
      | "file" -> _cfg_from_file rest
      | "etcd" -> let etcd = Etcd.parse_url rest in
                  _cfg_from_etcd etcd
      | _ -> (fun () -> Lwt.fail_with (Printf.sprintf "todo (protocol):%s" protocol))
    with End_of_file ->
      let () = Lwt_log.ign_warning_f "no protocol, assuming %s is a file" url in
      _cfg_from_file url
  in
  fetcher ()
