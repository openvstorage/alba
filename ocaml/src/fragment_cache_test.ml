...
let test_1 () =
  let _inner (cache :Fragment_cache.blob_cache) =
    cache # clear_all () >>= fun () ->
    let slice = Bytes.create 4096 in
    let blob = Blob.Slice (Slice.wrap_string slice) in
    ...
    >>= fun () ->
    Bytes.set slice 0 'y';     (* _replace_grow *)
    cache # add bid oid blob
    >>= fun () ->
    Bytes.set slice 0 'z';     (* _replace_grow *)
    ...
    cache # lookup bid oid >>=
      begin
        ...
        | None ->
           ...
           Lwt.return ()
        | Some retrieved ->
           OUnit.assert_bool "first byte?" ((Blob.to_string_unsafe retrieved).[0] = 'z');
...
let test_3 () =
  ...
  let _inner (cache: Fragment_cache.blob_cache) =
    cache # clear_all () >>= fun () ->
    let slice = Bytes.create blob_size in
    let blob = Blob.Slice (Slice.wrap_string slice) in
    cache # add 0l "X" blob >>= fun () -> (* _add_new_grow *)
...
let test_long () =
  let _inner (cache :Fragment_cache.blob_cache) =
    ...
    let blob = Blob.Slice (Slice.wrap_string "blob") in
    ...
    cache # drop 22l >>= fun () ->
    cache # drop 23l >>= fun () ->
    let make_blob () = Blob.Slice (Slice.wrap_string (Bytes.create (2048 + Random.int 2048))) in
    let fill n =
      let rec loop i =
        if i = 0
        then Lwt.return ()
