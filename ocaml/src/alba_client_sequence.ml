...
let _apply_sequence
  ...
  let upload object_name object_reader checksum_o =
    ...
    >>= fun (mf, extra_mfs, upload_stats, gc_epoch, fragment_state_layout) ->
    ...
    let all_mfs = (mf.Nsm_model.Manifest.name, "", (mf, namespace_id)) ::
                    (List.map
                       ...
                       extra_mfs)
    in
    Lwt.return (Nsm_model.Update.PutObject (mf, gc_epoch), all_mfs, `Upload (mf, gc_epoch, upload_stats, fragment_state_layout))
  in
  ...
  List.iter
    (function
     | `Upload (manifest, gc_epoch, upload_stats, fragment_state_layout) ->
